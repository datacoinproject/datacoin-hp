...
// extern int64 GetBlockValue(int nBits, int64 nFees);

int64 static GetBlockValueForExplorer(int nBits, int64 nFees)
{
    uint64 nSubsidy = 0;
    if (!TargetGetMint(nBits, nSubsidy))
        error("GetBlockValueForExplorer() : invalid mint value");
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    if (pBlock->nHeight == 0)
        ...
        Generated = GetBlockValueForExplorer(pBlock->nBits - 1, 0);
...
std::string TxToString(uint256 BlockHash, const CTransaction& tx)
{
    ...
    std::string Content;
    Content += "<h1 style=\"font-size:1.2em\">" + _("Transaction") + "&nbsp;<span class=\"mono\">" + Hash + "</span></h1>";
