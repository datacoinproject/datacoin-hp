...
void ProofOfImage::on_createPushButton_clicked()
{
    ...
    else if(pwalletMain->GetBalance() < 0.001)
    {
      ...
      error2box.setText("Error, You need at least 0.001 DTC to send proof of data!");
	  error2box.exec();
      ui->txLineEdit->setText("ERROR: You need at least a 0.001 DTC balance to send proof of data.");
    }
    else
    {
      //std::string sNarr;
      // std::string strError = pwalletMain->SendMoneyToDestination(baddr.Get(), nAmount, sNarr, wtx);
      std::string strError = std::string("Not Yet Implemented");
